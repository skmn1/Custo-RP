          ...
          {editing === entry.id ? null : (
            <>
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-sm font-semibold text-gray-900">{entry.positionTitle}</h3>
                  <p className="text-sm text-gray-600">{entry.companyName}</p>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => startEdit(entry)} className="text-xs text-indigo-600 hover:underline" data-cy="edit-experience">
                    {t('profile.experience.edit')}
                  </button>
                  <button onClick={() => handleDelete(entry.id)} className="text-xs text-red-600 hover:underline" data-cy="delete-experience">
...
const ExperienceForm = ({ form, setForm, saving, onSave, onCancel, t }) => (
  <form onSubmit={onSave} className="bg-white rounded-xl border border-gray-200 p-5 space-y-4" data-cy="experience-form">
    ...
    <div>
      <label className="block text-xs font-medium text-gray-500 mb-1">{t('profile.experience.description')}</label>
      <textarea
...
const FormInput = ({ label, value, onChange, type = 'text', required = false, disabled = false }) => (
  <div>
    ...
    <input
      ...
      required={required}
      disabled={disabled}
      className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg bg-white text-gray-900 focus:ring-2 focus:ring-indigo-500 focus:border-transparent disabled:opacity-50"
    />
  </div>
);
