...
const MonthView = ({ anchor, shifts, leave, isLoading }) => {
  ...
  return (
    <div className="rounded-xl border border-gray-200 overflow-hidden" data-testid="month-view">
      {/* Day-of-week headers */}
      ...
      {isLoading ? (
        <div className="grid grid-cols-7">
          {Array.from({ length: 35 }).map((_, i) => (
            <div key={i} className="h-16 border border-gray-100 bg-white">
              <div className="m-1 h-10 rounded bg-gray-100 animate-pulse" />
            </div>
          ))}
        </div>
      ) : (
            ...
            {cells.map((cell) => {
              ...
              return (
                <div
                  ...
                  className={`min-h-[64px] border border-gray-100 p-1 
                    ${isCurrentMonth ? 'bg-white' : 'bg-gray-50/50'}
                    ${(hasShifts || dayLeave.length > 0) ? 'cursor-pointer hover:bg-indigo-50/50' : 'cursor-default'}
                  ...
                  {dayShifts.length > 0 && (
                    <div className="flex flex-wrap gap-0.5 mt-0.5">
                      {dayShifts.map(s => (
                        <span
                          key={s.id}
                          className="w-2 h-2 rounded-full bg-indigo-500 block"
