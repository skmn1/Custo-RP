  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-semibold text-gray-900">{slip.periodLabel}</h3>
      ...
      <div className="grid grid-cols-3 gap-4 text-sm">
        <div>
          <p className="text-gray-500 text-xs">{t('payslips.grossPay')}</p>
          <p className="font-medium text-gray-900">{fmtCurrency(slip.grossPay, slip.currency)}</p>
        </div>
        <div>
          <p className="text-gray-500 text-xs">{t('payslips.deductions')}</p>
          <p className="font-medium text-red-600">−{fmtCurrency(slip.totalDeductions, slip.currency)}</p>
        </div>
        <div>
          <p className="text-gray-500 text-xs">{t('payslips.netPay')}</p>
          <p className="font-bold text-gray-900">{fmtCurrency(slip.netPay, slip.currency)}</p>
        </div>
      </div>
      {slip.paidAt && (
...
function YearFilter({ year, setYear, t }) {
  ...
  return (
    <select
      ...
      className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm text-gray-700 focus:ring-2 focus:ring-indigo-500"
