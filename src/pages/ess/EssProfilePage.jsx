  ...
  <div className="space-y-6">
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h2 className="text-lg font-semibold text-gray-900">{personal.name}</h2>
          <p className="text-sm text-gray-500">{contract.jobTitle} · {contract.department}</p>
        </div>
      </div>
...
const ChangeRequestsTab = ({ requests, queuedRequests = [], onCancel, t }) => {
  ...
  if (allRequests.length === 0) {
    return (
      <Card>
        <p className="text-sm text-gray-500">{t('profile.changeRequests.noRequests')}</p>
      </Card>
    );
  }
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm" data-cy="change-requests-table">
          ...
          <tbody className="divide-y divide-gray-100">
            {allRequests.map((r) => (
              <tr key={r.id}>
                <td className="py-2 pr-4 font-medium text-gray-900">{r.fieldLabel || r.fieldName}</td>
                <td className="py-2 pr-4 text-gray-500">{r.oldValue || '\u2014'}</td>
                <td className="py-2 pr-4 text-gray-900">{r.newValue}</td>
...
const SectionHeading = ({ children }) => (
  <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-4">{children}</h2>
);
